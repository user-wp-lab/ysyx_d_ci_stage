...
static struct rule {
  ...
  int token_type;
    ...
    /* TODO: Add more rules.
   * Pay attention to the recedence level of different rules.
   */
  {" +", TK_NOTYPE},   // spaces
  {"\\+", '+'},        // add
  {"-", '-'},          // substract
...
/*regexec函数中，re[i]指向编译好的，要执行的正则表达式，e+ positione匹配的源字符串
  ...
  while (e[position] != '\0') {
    ...
    for (i = 0; i < NR_REGEX; i ++) {
    //printf("%d",pmatch.rm_so);
    //printf("%d",regexec(&re[i], e + position, 1, &pmatch, 0));
...
//指针引用
  ...
  paddr_t pointer;
  int k_count = 0;
  if (tokens[i].type == '*' && (i == 0 || tokens[i - 1].type == '+' || tokens[i].type == '-')) {
      tokens[i].type = DEREF;
	if(tokens[i+1].type == DEC_NUM){
		pointer = strtol(tokens[i+1].str,NULL,10);
		return paddr_read(pointer,4);
	}else if(tokens[i+1].type == HEX_NUM){
		pointer = strtol(tokens[i+1].str,NULL,16);
		return paddr_read(pointer,4);
	}
	int t = 0;
      for(t = i+1; t < nr_token; t++){
       	  if(tokens[t].type == '('){
	  	k_count++;
	  }else if(tokens[t].type == ')'){
	  	k_count--;
	  }else if(k_count == 0 && t > i+1){
	  	break;
	  }
      }
      pointer = eval(i+1,t-1);
    return paddr_read(pointer,4);
  }
}
  ...
  //位运算及逻辑运算
    for(int i = 0;i < nr_token; i++){
  	if(tokens[i].type == TK_EQ || tokens[i].type == TK_NEQ || tokens[i].type == TK_AND || tokens[i].type == TK_OR || tokens[i].type == TK_LESS || tokens[i].type == TK_GREAT){
		word_t result_1 = eval(0,i-1);
		word_t result_2 = eval(i+1,nr_token-1);
                if(tokens[i].type == TK_EQ){
                      if(result_1 == result_2){return 1;}
                      else{return 0;}
                }else if(tokens[i].type == TK_NEQ){
                      if(result_1 == result_2){return 0;}
                      else{return 1;}
                }else if(tokens[i].type == TK_AND){
                      if(result_1 == 0 || result_2 == 0){return 0;}
                      else{return 1;}
                }else if(tokens[i].type == TK_OR){
                      if(result_1 == 0 && result_2 == 0){return 0;}
                      else{return 1;}
                }else if(tokens[i].type == TK_LESS){
		      if(result_1 <= result_2){return 1;}
		      else{return 0;}
		}else if(tokens[i].type == TK_GREAT){
		      if(result_1 >= result_2){return 1;}
		      else{return 0;}
		}
	}
